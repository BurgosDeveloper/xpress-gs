...
export async function createRide(params: {
  userId: string;
  serviceModeWanted: "TRASLADO" | "DELIVERY" | "ENVIO";
  ...
  const fixed = await getFixedZonePriceForTrip({
    ...
    serviceMode: params.serviceModeWanted as import("@prisma/client").ServiceMode,
  ...
  const ride = await prisma.rideRequest.create({
    data: {
      passengerId: passenger.id,
      serviceModeWanted: params.serviceModeWanted as import("@prisma/client").ServiceMode,
      ...
      estimatedPrice: params.offeredPrice ?? (fixed.ok ? fixed.amountCop : estimated),
  ...
  void sendPushToAdmins({
    ...
    body: `Ride ${ride.id} (${params.serviceModeWanted} - ${params.serviceTypeWanted})`,
...
export async function getRideByIdForRequester(params: {
  ...
  const ride = await prisma.rideRequest.findUnique({
    ...
    include: {
      ...
      addons: { include: { addon: true } },
      offer: { include: { committedDriver: { include: { user: { select: { id: true, email: true } } } } } },
      ratings: true,
    },
  });
  ...
  if (params.requester.role === "DRIVER") {
    if (ride.matchedDriver?.user?.id === params.requester.id) return { ok: true as const, ride };
    if (["OPEN", "REQUESTED", "OFFERED"].includes(ride.status)) return { ok: true as const, ride };
    if (ride.offer && ride.offer.committedDriver?.user?.id === params.requester.id) return { ok: true as const, ride };
  }
...
export async function offerRideForDriver(params: { userId: string; rideId: string; amount?: number }) {
  ...
  if (!["OPEN", "REQUESTED", "OFFERED"].includes(ride.status) || ride.matchedDriverId) {
  ...
  const candidate = existing
    ? await prisma.rideCandidate.update({
        ...
        data: { status: RideCandidateStatus.OFFERED, amount: params.amount },
      })
    : await prisma.rideCandidate.create({
        data: { rideId: ride.id, driverId: driver.id, status: RideCandidateStatus.OFFERED, amount: params.amount } as any,
      });
...
export async function listRideOffersForPassenger(params: { userId: string; rideId: string }) {
  ...
  const items = offered
    .map((c) => {
      const d = c.driver;
      if (!d || !d.user?.isActive) return null;
      ...
      return {
        ...
        serviceType: d.serviceType,
        vehicle: d.vehicle,
        driverLocation: loc ? { ...loc, updatedAt: d.location?.updatedAt } : null,
        distanceMeters: Math.round(dist),
...
export async function notifyPassengerArrived(params: { rideId: string; userId: string }) {
  const ride = await prisma.rideRequest.findUnique({
    where: { id: params.rideId },
    include: {
      passenger: { select: { userId: true } },
      matchedDriver: { select: { userId: true, fullName: true } },
    },
  });

  if (!ride) return { ok: false as const, status: 404 as const, error: "Ride not found" };
  if (ride.matchedDriver?.userId !== params.userId) {
    return { ok: false as const, status: 403 as const, error: "Forbidden" };
  }

  const passengerUserId = ride.passenger.userId;

  emitToUser(passengerUserId, "ride:arrived", { rideId: ride.id });
  emitToUser(passengerUserId, "ride:changed", { rideId: ride.id, type: "DRIVER_ARRIVED" });

  void sendPushToUser({
    userId: passengerUserId,
    title: "¡Tu ejecutivo ha llegado!",
    body: `${ride.matchedDriver.fullName || "Tu ejecutivo"} ya se encuentra en el punto de encuentro.`,
