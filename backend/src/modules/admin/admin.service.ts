...
import { DocumentType, DriverStatus, RideStatus, ServiceMode, ServiceType, UserRole } from "@prisma/client";
...
export async function adminGetAppConfig() {
  const [appConfig, pricingConfigs] = await Promise.all([
    getAppConfig(),
    prisma.pricingConfig.findMany({ orderBy: { serviceType: "asc" } }),
  ]);
  ...
  return {
    ...
    appConfig: {
      ...
      adminWhatsappPhone: text((appConfig as any).adminWhatsappPhone),
    ...
    serviceConfigs: pricingConfigs.map((p) => ({
      serviceType: p.serviceType,
      baseFare: Number(p.baseFare),
      perKmFare: Number(p.perKm),
      includedKm: Math.floor(p.includedMeters / 1000),
      matchingRadiusM: normalizeMatchingRadiusM(appConfig.matchingRadiusM), // Fallback to global for now
    })),
  };
}
...
export async function adminUpdateAppConfig(input: {
  ...
  const adminWhatsappPhone = sanitizeText(input.adminWhatsappPhone);
  ...
  const extraCreate: any = {
    ...
    ...(adminWhatsappPhone !== undefined ? { adminWhatsappPhone } : null),
  ...
  const updated = await prisma.$transaction(async (tx) => {
    ...
    if (input.serviceConfigs && input.serviceConfigs.length > 0) {
      for (const stConfig of input.serviceConfigs) {
        await tx.pricingConfig.upsert({
          where: { serviceMode_serviceType: { serviceMode: stConfig.serviceMode as ServiceMode, serviceType: stConfig.serviceType as ServiceType } },
          update: {
            baseFare: stConfig.baseFare,
            perKm: stConfig.perKmFare,
            includedMeters: stConfig.includedKm * 1000,
          },
          create: {
            serviceMode: stConfig.serviceMode as ServiceMode,
            serviceType: stConfig.serviceType as ServiceType,
            baseFare: stConfig.baseFare,
            perKm: stConfig.perKmFare,
            includedMeters: stConfig.includedKm * 1000,
            stepMeters: 0,
...
export async function upsertPricing(params: {
  serviceMode: "TRASLADO" | "DELIVERY" | "ENVIO";
  serviceType: "CARRO" | "MOTO" | "MOTO_CARGA" | "CARRO_CARGA";
  baseFare: number;
  ...
  return prisma.pricingConfig.upsert({
    where: { serviceMode_serviceType: { serviceMode: params.serviceMode as ServiceMode, serviceType: params.serviceType as ServiceType } },
    update: {
    ...
    create: {
      serviceMode: params.serviceMode as ServiceMode,
