...
export const UpsertPricingSchema = z.object({
  ...
  nightEndHour: z.coerce.number().int().min(0).max(23).optional().default(23),
...
export const UpdateAppConfigSchema = z.object({
  ...
  adminWhatsappPhone: z.string().max(40).optional().nullable(),

  serviceConfigs: z.array(
    z.object({
      serviceMode: z.enum(["TRASLADO", "DELIVERY", "ENVIO"]),
      serviceType: z.enum(["CARRO", "MOTO", "MOTO_CARGA", "CARRO_CARGA"]),
      baseFare: z.coerce.number().nonnegative(),
      perKmFare: z.coerce.number().nonnegative(),
      includedKm: z.coerce.number().int().nonnegative(),
      matchingRadiusM: z.coerce.number().int().positive().max(20000),
    })
  ).optional(),
