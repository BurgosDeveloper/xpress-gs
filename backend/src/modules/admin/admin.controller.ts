...
export async function adminUpsertPricingController(req: Request, res: Response) {
  const serviceMode = String(req.params.serviceMode);
  const serviceType = String(req.params.serviceType);
  const allowedModes = ["TRASLADO", "DELIVERY", "ENVIO"] as const;
  const allowedTypes = ["CARRO", "MOTO", "MOTO_CARGA", "CARRO_CARGA"] as const;
  if (!allowedModes.includes(serviceMode as any) || !allowedTypes.includes(serviceType as any)) {
    return res.status(400).json({ message: "Invalid serviceMode or serviceType" });
  }

  const input = UpsertPricingSchema.parse(req.body);
  const row = await upsertPricing({ serviceMode: serviceMode as any, serviceType: serviceType as any, ...input });
