import { DriverStatus, OfferStatus, RideStatus, ServiceMode, ServiceType } from "@prisma/client";
...
export async function createOffer(params: {
  ...
  if (committedRide) {
    return {
      ok: false as const,
      status: 409 as const,
      error: "Ya tenés un servicio en curso o con chofer asignado. Cancelalo antes de crear una contraoferta.",
    };
  }

  // Si tiene solicitudes u ofertas previas aún en estado OPEN (sin chofer asignado),
  // las cancelamos automáticamente para que nunca quede bloqueado.
  ...
  const estimation = await estimateOffer({
    serviceModeWanted: params.serviceModeWanted,
...
export async function commitOffer(params: { userId: string; offerId: string; coords?: { lat: number; lng: number } }) {
  ...
  const pricing = await prisma.pricingConfig.findUnique({ where: { serviceMode_serviceType: { serviceMode: offer.serviceModeWanted, serviceType: pricingType } } });
  if (!pricing) return { ok: false as const, error: "Pricing not configured for this service type" };
