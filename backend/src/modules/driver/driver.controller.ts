...
import { ServiceMode, ServiceType } from "@prisma/client";
...
export async function driverUpdateMeterController(req: Request, res: Response) {
  ...
  const pricing = await prisma.pricingConfig.findUnique({ where: { serviceMode_serviceType: { serviceMode: ride.serviceModeWanted as ServiceMode, serviceType: pricingType } } });
