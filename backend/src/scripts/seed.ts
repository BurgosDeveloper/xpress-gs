...
async function ensurePricing() {
  ...
  for (const row of rows) {
    await prisma.pricingConfig.upsert({
      where: { serviceMode_serviceType: { serviceMode: "TRASLADO", serviceType: row.serviceType } },
      create: {
...
async function main() {
  ...
  try {
    const deletedCount = await prisma.user.deleteMany({
      where: {
        OR: [
          { email: { contains: "xpress" } },
          { email: { contains: "local" } },
          { email: { contains: "start" } },
          { username: { contains: "xpress" } },
          { username: { contains: "start" } }
        ]
      }
    });
    // eslint-disable-next-line no-console
    console.log("[seed] cleaned up old accounts", deletedCount);
  } catch (err) {
    // eslint-disable-next-line no-console
    console.error("[seed] failed to clean up old accounts", err);
  }

  const username = "admin";
  const email = "admin@gs.com";
  const password = "gs_admin";
  ...
  const passengerEmail = "cliente@gs.com";
  const passengerPassword = "gs_test";
  ...
  const driverEmail = "chofer@gs.com";
  const driverUsername = "chofer";
  const driverPassword = "gs_test";
