...
import { apiCancelRide, apiDriverNearbyRideRequests, apiDriverOfferRide, apiGetActiveRide, apiGetRideById, apiGetRideOffers, apiDriverRideHistory } from "../rides/rides.api";
...
import { PassengerHomeView } from "./PassengerHomeView";
...
export function HomeScreen({ navigation }: Props) {
  ...
  useEffect(() => {
    ...
    // Polling eliminado en favor de Sockets (Hito 2)
  ...
  const handleRealtimeRideChange = (payload: any) => {
    ...
    if ((role as any) === "DRIVER" || role === "USER") {
      void handleRideChangedForegroundRealtime({ role: role as "DRIVER" | "USER", payload });
    }

    if ((role as any) === "DRIVER" && payloadRideId && (payloadType === "RIDE_ASSIGNED" || payloadType === "RIDE_MATCHED")) {
  ...
  // Track driver location passively while on Home Screen
  useEffect(() => {
    if (!token) return;
    if (role !== "DRIVER") return;
      ...
      try {
        ...
        sub = await Location.watchPositionAsync(
          ...
          (loc) => {
            ...
            apiDriverUpsertLocation(token!, { lat, lng }).catch(() => {});
          }
        );
      } catch {
        // ignore
  ...
  // Hide default navigation header for all roles on HomeScreen
  useEffect(() => {
    navigation.setOptions({ headerShown: false });
  }, [navigation]);

  // Fetch driver real database metrics
  useEffect(() => {
    if (!token || role !== "DRIVER" || !isFocused) return;
    ...
    async function loadDriverMetrics() {
      try {
        const [creditsRes, historyRes] = await Promise.all([
          apiGetMyCredits(token!),
          apiDriverRideHistory(token!),
        ]);

        if (!alive) return;

        if (creditsRes && creditsRes.ok) {
          setDriverBalance(creditsRes.balanceCop || 0);
        }

        if (historyRes && historyRes.ok && Array.isArray(historyRes.rides)) {
          const completed = historyRes.rides.filter((r: any) => r.status === "COMPLETED");
          setCompletedRidesCount(completed.length);
          
          const today = new Date().toDateString();
          const earningsToday = completed
            .filter((r: any) => new Date(r.updatedAt).toDateString() === today)
            .reduce((sum: number, r: any) => sum + (Number(r.agreedPrice) || 0), 0);
          setDriverEarnings(earningsToday);
        }
      } catch (err) {
        // ignore
      }
    ...
    void loadDriverMetrics();

    return () => {
      alive = false;
    };
  }, [token, role, isFocused]);
  ...
  const ratingDirection = role === "USER" ? "PASSENGER_TO_DRIVER" : (role as any) === "DRIVER" ? "DRIVER_TO_PASSENGER" : null;
  ...
  const meterPriceToShow = useMemo(() => {
    ...
    if (useSteps) {
      ...
      const extraKm = Math.max(0, kmWhole - Math.floor(includedMeters / 1000));
  ...
  }, [attentionRide, meterDistanceMeters, role]);
  ...
  if (role === "USER") {
    return (
      <PassengerHomeView
        ...
        userHasOpenOffer={userHasOpenOffer}
        offersRideId={offersRideId}
        offersRideLoading={offersRideLoading}
        offersRideCount={offersRideCount}
        rideActionLoading={rideActionLoading}
        onNavigateMakeOffer={(serviceMode) => navigation.navigate("PassengerDriversMap", { serviceModeWanted: serviceMode, serviceTypeWanted: "CARRO" })}
        onNavigateOfferLibre={() => navigation.navigate("PassengerMakeOffer", {})}
        onNavigateOffersWait={(rideId) => navigation.navigate("PassengerOffersWait", { rideId })}
        onNavigateHistory={() => navigation.navigate("RidesHistory")}
        onNavigateRideDetails={(rideId) => navigation.navigate("RideDetails", { rideId })}
        onNavigateChat={(rideId) => navigation.navigate("Chat", { rideId })}
  ...
  if (role === "ADMIN") {
    return (
      <AdminHomeView
        onNavigateSettings={() => navigation.navigate("AdminSettings")}
        onNavigateZones={() => navigation.navigate("AdminZones")}
        onNavigateDrivers={() => navigation.navigate("AdminDrivers")}
        onNavigatePassengers={() => navigation.navigate("AdminPassengers")}
        onNavigateRides={() => navigation.navigate("AdminRides")}
        onNavigatePasswordResets={() => navigation.navigate("AdminPasswordResets")}
        onOpenProfile={() => navigation.navigate("Profile")}
