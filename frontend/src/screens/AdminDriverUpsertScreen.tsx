...
export function AdminDriverUpsertScreen({ route, navigation }: Props) {
  ...
  const title = useMemo(() => (isEdit ? "Editar ejecutivo" : "Agregar ejecutivo"), [isEdit]);
  ...
  async function save() {
    ...
    if (!photoUrl.trim()) {
      Alert.alert("Faltan datos", "Subí la foto del ejecutivo");
      return;
    }
      ...
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        <View style={styles.headerRow}>
          <View style={styles.headerLeft}>
            <Ionicons name={isEdit ? "create-outline" : "person-add-outline"} size={20} color={colors.neon} />
          ...
          <Card style={{ marginTop: 12, gap: 10 }}>
            ...
            <View style={{ flexDirection: "row", gap: 10, flexWrap: "wrap" }}>
              <Pressable onPress={() => void toggleActive()} style={({ pressed }) => [styles.actionBtn, pressed && styles.pressed]} disabled={saving}>
                <Ionicons name={isActive ? "pause-circle-outline" : "play-circle-outline"} size={18} color={isActive ? colors.danger : colors.neon} />
        ...
        <Card style={{ marginTop: 12, gap: 12 }}>
          ...
          <View style={{ gap: 10 }}>
            <Text style={styles.label}>Foto ejecutivo</Text>
