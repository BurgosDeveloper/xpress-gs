...
import { apiDriverNearbyRideRequests } from "../rides/rides.api";
...
import { handleDriverNearbyForegroundRealtime } from "../notifications/foregroundRealtime";
...
export function DriverOffersListScreen({ navigation }: Props) {
  ...
  async function refresh() {
    ...
    try {
      ...
      const res = await apiDriverNearbyRideRequests(token, { radiusM: matchingRadiusM, take: 50 });
      if (mySeq !== refreshSeq.current) return;
      setItems(res.items);
    } catch (e) {
      if (mySeq !== refreshSeq.current) return;
    ...
    <Screen>
      <ScrollView contentContainerStyle={{ paddingBottom: 24 }} keyboardShouldPersistTaps="handled">
        ...
        <View style={{ gap: 12, marginTop: 16 }}>
          {items.map((r) => (
            <Pressable key={r.rideId} onPress={() => navigation.navigate("DriverOfferDetails", { offerId: r.rideId })}>
              <Card style={{ gap: 8 }}>
                <View style={styles.itemTitleRow}>
                  <Ionicons name="location-outline" size={16} color={colors.neon} />
                  <Text style={styles.itemTitle}>Viaje {serviceTypeLabel(r.serviceTypeWanted)}</Text>
                </View>
                {r.passenger?.fullName ? <Text style={styles.small}>Cliente: {r.passenger.fullName}</Text> : null}
                {r.passenger?.phone ? <Text style={styles.small}>Teléfono: {r.passenger.phone}</Text> : null}
                <Text style={styles.small}>Distancia a recoger: {Math.round(r.distanceMeters)} m</Text>
                <Text style={styles.small}>Distancia viaje: {Math.round(r.rideDistanceMeters)} m</Text>
                <Text style={styles.small}>Oferta del Pasajero: {formatCop(r.estimatedPrice)}</Text>
                ...
                <Text style={styles.small}>Salida: {r.pickup.address ?? `${r.pickup.lat.toFixed(4)}, ${r.pickup.lng.toFixed(4)}`}</Text>
                <Text style={styles.small}>Destino: {r.dropoff.address ?? `${r.dropoff.lat.toFixed(4)}, ${r.dropoff.lng.toFixed(4)}`}</Text>
              </Card>
            </Pressable>
          ))}
