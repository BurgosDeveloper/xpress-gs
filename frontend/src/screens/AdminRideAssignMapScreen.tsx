...
export function AdminRideAssignMapScreen({ route, navigation }: Props) {
  ...
  async function assign() {
    ...
    try {
      ...
      Alert.alert("Listo", "Ejecutivo asignado.");
      navigation.goBack();
    } catch (e) {
      setError(e instanceof Error ? e.message : "No se pudo asignar el ejecutivo");
    } finally {
      setAssigning(false);
    }
  }
  ...
  const markers = useMemo(() => {
    const items: AppMapMarker[] = [{ id: "pickup", coordinate: toLatLng(pickup), pinColor: colors.neon }];
    ...
    for (const d of drivers) {
      ...
      items.push({
        ...
        pinColor: isSel ? colors.neon : colors.danger,
    ...
    <Screen style={{ padding: 0 }}>
      <View style={styles.topBar}>
        <View style={styles.topBarLeft}>
          <Ionicons name="pin-outline" size={18} color={colors.neon} />
      ...
      <View style={styles.mapWrap}>
        ...
        {loading ? (
          <View style={styles.loadingOverlay}>
            ...
            <Text style={styles.loadingText}>Cargando ejecutivos...</Text>
          </View>
        ) : null}
        ...
        <View style={styles.sheet}>
          <Card style={{ gap: 10 }}>
            ...
            <Text style={styles.sheetLine}>Ejecutivos en mapa: {drivers.length}</Text>
            ...
            {selectedDriver ? (
              ...
              <Text style={styles.sheetMuted}>Toca un ejecutivo en el mapa para seleccionarlo.</Text>
            )}

            <PrimaryButton
              label={assigning ? "Asignando..." : "Asignar ejecutivo"}
              onPress={() => void assign()}
