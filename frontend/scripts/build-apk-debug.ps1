...
$envContent = Get-Content -Path (Join-Path $PSScriptRoot '..\.env')
$mapboxToken = $envContent | Where-Object { $_ -match '^MAPBOX_DOWNLOADS_TOKEN=(.+)$' } | ForEach-Object { $Matches[1] }
"sdk.dir=$($sdkRoot -replace '\\','/')" | Set-Content -Encoding ASCII -Path $localProps
if ($mapboxToken) {
  Add-Content -Path $localProps -Value "MAPBOX_DOWNLOADS_TOKEN=$mapboxToken"
  $env:MAPBOX_DOWNLOADS_TOKEN = $mapboxToken
}
